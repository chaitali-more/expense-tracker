...
const DashboardPage = ({ Transactions = [], balance, totalIncome, totalExpense }) => {
  ...
  const incomeList = sortedTransactions.filter((tx) => tx.transactionType === "income");
  const expenseList = sortedTransactions.filter((tx) => tx.transactionType === "expense");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 bg-clip-text text-transparent mt-0.5">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            <Link
              to="/add-income"
              className="px-4 py-2 text-xs font-bold rounded-full bg-emerald-600 hover:bg-emerald-700 text-white shadow-md shadow-emerald-500/20 transition cursor-pointer flex items-center gap-1.5"
            >
              <HiSparkles className="w-3.5 h-3.5" /> Smart Finance AI
            </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-5 sm:p-7 rounded-2xl border border-emerald-200/80 shadow-xs hover:shadow-md transition-all space-y-5 relative overflow-hidden flex flex-col justify-between">
          <div className="h-1.5 w-full bg-gradient-to-r from-emerald-500 to-teal-500 absolute top-0 left-0 rounded-t-2xl" />
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              ...
              <span className="text-[11px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200/60 shrink-0">
            ...
            {recentIncomes.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {recentIncomes.map((tx) => (
                  <div
                    key={tx.id}
                    className="flex items-center justify-between p-3.5 rounded-xl bg-emerald-50/40 border border-emerald-100/70 hover:bg-emerald-50/80 transition"
                  >
          ...
          <Link
            to="/add-income"
            className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-4 rounded-xl text-xs shadow-md shadow-emerald-500/20 transition cursor-pointer flex items-center justify-center gap-2 text-center mt-2"
          >
        ...
        <div className="bg-white p-5 sm:p-7 rounded-2xl border border-red-200/80 shadow-xs hover:shadow-md transition-all space-y-5 relative overflow-hidden flex flex-col justify-between">
          <div className="h-1.5 w-full bg-gradient-to-r from-red-600 to-rose-500 absolute top-0 left-0 rounded-t-2xl" />
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              ...
              <span className="text-[11px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-full bg-red-50 text-red-700 border border-red-200/60 shrink-0">
            ...
            {recentExpenses.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {recentExpenses.map((tx) => (
                  <div
                    key={tx.id}
                    className="flex items-center justify-between p-3.5 rounded-xl bg-red-50/40 border border-red-100/70 hover:bg-red-50/80 transition"
                  >
                    <div className="space-y-0.5">
                      <span className="font-bold text-slate-800 text-xs block">
                        {tx.transactionName}
          ...
          <Link
            to="/add-expense"
            className="w-full bg-gradient-to-r from-red-600 via-red-500 to-rose-600 hover:opacity-95 text-white font-bold py-3 px-4 rounded-xl text-xs shadow-md shadow-red-500/20 transition cursor-pointer flex items-center justify-center gap-2 text-center mt-2"
          >
            <HiPlus className="w-4 h-4" />
